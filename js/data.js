...
const STATS = [
  ...
  },
  {
    ...
    trait: '品格・所作',
    parfait: 'レースの席と金縁の器',
    desc: '磨くほど席が整い、金の縁・受け皿・スプーンが順に加わる。',
    color: '#c9a227',
    color2: '#f2e0a0',
...
/**
 * レベルごとの「見た目の変化」。ステータス欄に次の変化として表示し、
 * パフェの描画（parfait.js）の解禁レベルと対応させている。
 */
const STAGES = {
  muscle: [
    [1, 'てっぺんに生クリームが載る'],
...
/** そのレベルの次に起きる変化（なければ null） */
function nextStage(key, level) {
  const list = STAGES[key] || [];
  for (const [lv, text] of list) if (level < lv) return { lv, text };
  return null;
}
